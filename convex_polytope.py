...
# img = np.asarray(cv2.cvtColor(img, cv2.COLOR_BGR2RGB), dtype=np.float32)
...
blur_img = 0.25*(np.roll(img,1,1) + np.roll(img,-1,1) + np.roll(img,1,0) + np.roll(img,-1,0))
img = np.log(np.log(img))
blur_img = np.log(np.log(blur_img))
avg_img = (blur_img + np.roll(blur_img,1,2) + np.roll(blur_img,2,2))/3.0
